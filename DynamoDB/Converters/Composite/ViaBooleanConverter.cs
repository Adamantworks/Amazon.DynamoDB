...
{
	public class ViaBooleanConverter : ValueConverter<DynamoDBString>
	{
		public override bool CanConvertTo(Type toType, IValueConverter context)
		{
			return context.CanConvert(typeof(DynamoDBBoolean), toType, context);
		}
		...
		{
			// Can't make a boolean in these cases
			bool boolValue;
			if(fromValue == null || !bool.TryParse(fromValue.ToString(), out boolValue))
			{
				toValue = null;
				return false;
			}
