...
namespace Adamantworks.Amazon.DynamoDB
{
	public class PredicateExpression : IAwsAttributeNameProvider, IAwsAttributeValuesProvider
	{
		...
		public PredicateExpression(string expression, Values values)
		{
			Expression = expression;
			this.values = values.DynamoDBMapDeepClone();
			names = new Dictionary<string, string>();
		}
		public PredicateExpression(string expression, Values values, IEnumerable<KeyValuePair<string, string>> names)
		{
			Expression = expression;
			this.values = values.DynamoDBMapDeepClone();
			this.names = AwsAttributeNames.Build(names);
		}
		public PredicateExpression(string expression, Values values, params string[] names)
		{
			Expression = expression;
			this.values = values.DynamoDBMapDeepClone();
			this.names = AwsAttributeNames.Build(names);
		}
		public PredicateExpression(string expression, Values values, object names)
		{
			Expression = expression;
			this.values = values.DynamoDBMapDeepClone();
			this.names = AwsAttributeNames.Build(names);
