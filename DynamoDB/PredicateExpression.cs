...
{
	public class PredicateExpression : IAwsAttributeNameProvider, IAwsAttributeValuesProvider
	{
		...
		public PredicateExpression(string expression, IEnumerable<KeyValuePair<string, string>> names)
		{
			Expression = expression;
			this.names = AwsAttributeNames.Build(names);
		}
		public PredicateExpression(string expression, params string[] names)
		{
			Expression = expression;
			this.names = AwsAttributeNames.Build(names);
		}
		public PredicateExpression(string expression, object names)
		{
			Expression = expression;
			this.names = AwsAttributeNames.Build(names);
		}
		public PredicateExpression(string expression, Constants constants)
		{
			Expression = expression;
			this.constants = constants.DynamoDBMapDeepClone();
			names = new Dictionary<string, string>();
		}
		public PredicateExpression(string expression, Constants constants, IEnumerable<KeyValuePair<string, string>> names)
		{
			Expression = expression;
			this.constants = constants.DynamoDBMapDeepClone();
			this.names = AwsAttributeNames.Build(names);
		}
		public PredicateExpression(string expression, Constants constants, params string[] names)
		{
			Expression = expression;
			this.constants = constants.DynamoDBMapDeepClone();
			this.names = AwsAttributeNames.Build(names);
		}
		public PredicateExpression(string expression, Constants constants, object names)
		{
			Expression = expression;
			this.constants = constants.DynamoDBMapDeepClone();
			this.names = AwsAttributeNames.Build(names);
